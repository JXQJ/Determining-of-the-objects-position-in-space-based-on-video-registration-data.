...
CalibrateImagesWindow::CalibrateImagesWindow(QWidget *parent) : QDialog(parent)
{
	boardWidth = new QLabel("Board Width");
	lineWidth = new QLineEdit;

	boardHeight = new QLabel("Board Height");
	lineHeight = new QLineEdit;

	squareSizeLabel = new QLabel("Square size label");
	squareSizeEdit = new QLineEdit;
	...
	windowHorizontalSquareSize = new QHBoxLayout;
	windowHorizontalPathToSave = new QHBoxLayout;
	windowHorizontalWidth = new QHBoxLayout;
	windowHorizontalHeight = new QHBoxLayout;

	windowHorizontalWidth->addWidget(boardWidth);
	windowHorizontalWidth->addWidget(lineWidth);

	windowHorizontalHeight->addWidget(boardHeight);
	windowHorizontalHeight->addWidget(lineHeight);

	windowHorizontalSquareSize->addWidget(squareSizeLabel);
	windowHorizontalSquareSize->addWidget(squareSizeEdit);

	windowHorizontalPathToSave->addWidget(pathToSaveLabel);
	windowHorizontalPathToSave->addWidget(pathToSaveEdit);

	windowVertical->addLayout(windowHorizontalHeight);
	windowVertical->addLayout(windowHorizontalWidth);
	windowVertical->addLayout(windowHorizontalSquareSize);
...
void CalibrateImagesWindow::StartClicked()
{
	bool displayCorners = false;
	bool showUndistorted = true;
	bool isVerticalStereo = false; // horiz or vert cams
	const int maxScale = 1;
	const float squareSize = 2.71f;
	//string str = pathToSaveEdit->text().split("/n")[0].toStdString();
	// actual square size
	string str = pathToSaveEdit->text().split("/n")[0].toStdString();
	string str2 = str;
	//string str = qstr.split("/n")[0].toStdString();
	const char *imageList = str.append("data.txt").c_str();
	FILE *f = fopen(imageList, "rt");
	int i, j, lr;
	int nx = lineWidth->text().split("/n")[0].toInt();
	int ny = lineHeight->text().split("/n")[0].toInt();
	int N = nx * ny;
	...
	QMessageBox msg;
	msg.setText("Calibrating is start. Click OK");
	msg.exec();
	...
	QMessageBox msg2;
	msg2.setText("Calibrating is done. Click OK");
	msg2.exec();
	...
	if (showUndistorted) {
		...
		if (true) {
			...
			str2.append("stereocalib.yml");
			cv::FileStorage fs(str2, cv::FileStorage::WRITE);
			fs << "CM1" << M1;
			fs << "CM2" << M2;
			fs << "D1" << D1;
			fs << "D2" << D2;
...
//
//			cout << "Starting to store results" << endl;
//			cv::FileStorage fs("stereocalib.yml", cv::FileStorage::WRITE);
//			fs << "CM1" << M1;
//			fs << "CM2" << M2;
//			fs << "D1" << D1;
//			fs << "D2" << D2;
